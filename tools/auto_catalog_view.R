...
compare <- function(catalogName, viewName, path=getwd()) {
  views <- xmlTreeParse("views/demo-views.xml") #import views
  ...
  catalog <- xmlTreeParse(paste0("questions/", catalogName, ".xml")) #import specified catalog
  catalog <- paste0(as.character(catalog), collapse = "") #creates character with length=1
  attributes <- xmlTreeParse("domain/rdmo.xml") #import attributes
  attribute <- character() 
